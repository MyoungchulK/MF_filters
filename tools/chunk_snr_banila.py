...
def snr_banila_collector(Data, Ped, analyze_blind_dat = False, use_l2 = False, no_tqdm = False):

    print('Collecting snr banila starts!')
    if use_l2:
    ...
    # wf analyzer
    wf_int = wf_analyzer(use_time_pad = True, use_band_pass = False, use_cw = False, use_l2 = use_l2, analyze_blind_dat = analyze_blind_dat, st = st, run = run)
    del st, run
    ...
    for evt in tqdm(range(num_evts), disable = no_tqdm):
        ...
        for ant in range(num_ants):
            ...
            wf_int.get_int_wf(raw_t, raw_v, ant, use_band_pass = False, use_cw = False, use_p2p = True, evt = evt)
    ...
    print('SNR banila collecting is done!')
