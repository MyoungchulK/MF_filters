...
def rayl_nb_collector(Data, Ped, analyze_blind_dat = False, use_l2 = False, use_sim = False, no_tqdm = False):

    print('Collecting rayl. no band pass starts!')
    ...
    # wf analyzer
    wf_int = wf_analyzer(use_time_pad = True, use_freq_pad = True, use_band_pass = False, use_rfft = True, use_cw = True, use_l2 = use_l2, analyze_blind_dat = analyze_blind_dat, st = st, run = run)
    ...
    for evt in tqdm(range(num_clean_softs), disable = no_tqdm):
        ...
        for ant in range(num_ants):
            ...
            wf_int.get_int_wf(raw_t, raw_v, ant, use_zero_pad = True, use_band_pass = False, use_cw = True, evt = clean_soft_entry[evt])
            del raw_t, raw_v 
            ara_root.del_TGraph()
    ...
    print('Rayl. no band pass collecting is done!')
