...
class pre_qual_cut_loader:
    ...
    def run_pre_qual_cut(self, use_for_ped_qual = False):
        ...
        tot_pre_qual_cut = np.full((self.num_evts, 18), 0, dtype = int)
        ...
        if use_for_ped_qual == False:
            tot_pre_qual_cut[:, 16:] = self.get_pedestal_block_events(apply_daq_err = np.nansum(tot_pre_qual_cut[:, :5], axis = 1))
