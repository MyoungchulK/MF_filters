...
def savgol_collector(Data, Ped, analyze_blind_dat = False, use_l2 = False):
    ...
    # wf analyzer
    wf_int = wf_analyzer(use_time_pad = True, use_freq_pad = True, use_band_pass = True, use_rfft = True, use_cw = True)

    # output
    freq_bins = np.linspace(0, 1, 200 + 1)
    freq_bin_center = (freq_bins[1:] + freq_bins[:-1]) / 2
    ratio_bins = np.linspace(-2, 1, 300 + 1)
    ...
    for evt in tqdm(range(num_evts)):
        ...
        for ant in range(num_ants):
            ...
            ratio = 1 - sav / ffts[:pad_num[ant], ant]
