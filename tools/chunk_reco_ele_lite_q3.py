...
def reco_ele_lite_q3_collector(Data, Ped, analyze_blind_dat = False, use_l2 = False, no_tqdm = False):
    ...
    q_num_evts = num_evts // 4 + half_num_evts
    sel_range = np.arange(num_evts, dtype = int)
    sel_evt_idx = np.logical_and(sel_range > half_num_evts1, sel_range < q_num_evts).astype(int)

    # loop over the events
    for evt in tqdm(range(num_evts), disable = no_tqdm):
      #if evt == 0:        
      if evt > half_num_evts1 and evt < q_num_evts:       
