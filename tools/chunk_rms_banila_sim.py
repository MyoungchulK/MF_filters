...
def rms_banila_sim_collector(Data, Station, Year):

    print('Collecting rms banila sim starts!')
    ...
    wf_int = wf_analyzer(use_time_pad = True, use_band_pass = False, use_cw = False, verbose = True, new_wf_time = wf_time, sim_path = band_path)
    ...
    for evt in tqdm(range(num_evts)):
        ...
        for ant in range(num_ants):
            wf_int.get_int_wf(wf_time, wf_v[:, ant], ant, use_sim = True, use_band_pass = False, use_p2p = True, use_cw = False, evt = evt)
    ...
    print('RMS banila collecting is done!')
