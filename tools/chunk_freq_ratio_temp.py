...
def freq_ratio_temp_collector(Data, Ped, analyze_blind_dat = False):
    ...
    del ara_uproot, run_info, qual_dat, qual_hf, qual_evt, qual_daq
    ...
    fft = np.full((len(freq_range), num_ants, 10), np.nan, dtype = float)
    fft_notch = np.copy(fft)
    count = 0

    # loop over the events
    for evt in tqdm(range(num_evts)):
      #if evt > 1000 and evt < 1011:
      if evt == np.where(evt_num == 63650)[0][0]: 
    ...
    del ara_root, num_evts, num_ants, wf_int, daq_cut, num_params
    ...
    return {'evt_num':evt_num,
            ...
            'freq_range_notch':freq_range_notch,
            'fft':fft,  
            'fft_notch':fft_notch,  
            'notch_idx_freq':notch_idx_freq,  
            'notch_idx':notch_idx,  
            'df':np.array([df]),  
