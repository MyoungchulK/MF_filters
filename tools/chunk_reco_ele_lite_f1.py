...
def reco_ele_lite_f1_collector(Data, Ped, analyze_blind_dat = False, use_l2 = False, no_tqdm = False):

    print('Collecting reco ele lite first half starts!')
    ...
    sel_evt_idx = (np.arange(num_evts, dtype = int) < half_num_evts).astype(int)

    # loop over the events
    for evt in tqdm(range(num_evts), disable = no_tqdm):
      #if evt == 0:        
      if evt < half_num_evts:       
    ...
    print('Reco ele lite first half collecting is done!')
