...
class py_interferometers:
    ...
    def get_arrival_time_tables(self):
        ...
        self.table = np.full((len(theta), len(phi), self.pair_len), np.nan, dtype = float)
        ...
        for p in range(self.pair_len):
            ...
            self.table[:, :, p] = arr_table[:, :, 0, p_1st, 0] - arr_table[:, :, 0, p_2nd, 0]
            table_p1[:, :, p] = arr_table[:, :, 0, p_1st, 0]
            table_p2[:, :, p] = arr_table[:, :, 0, p_2nd, 0]
            del p_1st, p_2nd
    ...
    def get_coval_sample(self, sum_pol = False):
        ...
        if sum_pol:
            corr_v_sum = np.nansum(self.coval[:, :, :self.v_pairs_len], axis = 2)
            corr_h_sum = np.nansum(self.coval[:, :, self.v_pairs_len:], axis = 2)

            corr_v_max = np.nanmax(corr_v_sum)
            corr_h_max = np.nanmax(corr_h_sum)
            self.coval = np.asarray([corr_v_max, corr_h_max]) # pol, rad, sol

            self.coord = np.full((2, 2), np.nan, dtype = float) # pol, thetapi, rad, sol
            self.coord[0] = np.asarray(np.where(corr_v_sum == corr_v_max), dtype = float).flatten()                 
            self.coord[1] = np.asarray(np.where(corr_h_sum == corr_h_max), dtype = float).flatten()
