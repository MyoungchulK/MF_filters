...
class sin_subtract_loader:
    ...
    def __init__(self, max_fail_atts = 3, min_freq = 0.2, max_freq = 0.3, dt = 0.5, cw_params = None):
        ...
        self.sin_sub = ROOT.FFTtools.SineSubtract(max_fail_atts, False) # no store
        #self.sin_sub.setVerbose(True)
        self.sin_sub.setFreqLimits(min_freq, max_freq)
        #self.sin_sub.unsetFreqLimits()

        self.min_params = cw_params[0]
        self.min_params = self.min_params.astype(np.double)
        self.amp_err_params = cw_params[1]
        self.amp_err_params = self.amp_err_params.astype(np.double)
        self.phase_err_params = cw_params[2]
        self.phase_err_params = self.phase_err_params.astype(np.double)

    def get_sin_subtract_wf(self, int_v, int_num, ant):

        int_v = int_v.astype(np.double)
        cw_v = np.full((int_num), 0, dtype = np.double)
        self.sin_sub.subtractCW(int_num, int_v, self.dt, cw_v, self.min_params[ant], self.amp_err_params[ant], self.phase_err_params[ant])
        ...
        #self.sub_tot_ratios = 1 - self.sub_powers[0] / self.sub_powers[-1]
