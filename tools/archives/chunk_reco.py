...
def reco_collector(Data, Ped):
    ...
    # interferometers
    ara_int = py_interferometers(wf_int.dt, 41, ara_uproot.station_id, ara_uproot.year, ara_uproot.run)
    ...
    for evt in tqdm(range(num_evts)):
        ...
        time_arr = buffer_info.get_time_arr(blk_idx_arr, trim_1st_blk = True, use_int_dat = True, return_min_max = True)
        wf_int.get_time_pad(dynamic_pad_len = time_arr, fill_pad = np.nan)
        del blk_idx_arr, time_arr
        ...
        wf_int.del_time_pad()
    del ara_root, ara_uproot, buffer_info, num_evts, num_ants, clean_evt_idx, wf_int, ara_int
