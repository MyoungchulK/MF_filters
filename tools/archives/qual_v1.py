...
def roll_mean_max_finder(i_v_pol, dt, SAMPLES_PER_BLOCK = SAMPLES_PER_BLOCK):
    ...
    if len(max_index) > 0:
        roll_mm = np.array([roll_v[max_index[0]], max_index[0]*dt])
    else:
        print('Index Issue!')
...
def mean_blk_finder(raw_v, even_odd_samp, even_odd_blk):
    ...
    samp_in_blk = even_odd_samp[even_odd_blk]
...
def freq_glitch_finder(int_v, dt):
    ...
    int_v_len = len(int_v)
    fft = np.abs(np.fft.rfft(int_v)/np.sqrt(int_v_len))
    fft_max = np.nanmax(fft)
    peak_idx = np.where(fft == fft_max)[0]
    if len(peak_idx) > 0:
         freq_glitch = np.array([peak_idx[0]/(int_v_len*dt),fft_max])
    else:
         freq_glitch = np.full((2),np.nan)
    del int_v_len, fft, fft_max, peak_idx
...
def freq_glitch_error_chunk(freq_glitch, Station, unix_time):
    ...
    fb_idx = np.where(qual_num_tot != 0)[0]
    if len(fb_idx) > 0:
        print('Qcut, freq glitch (entry):',fb_idx)
    else:
        pass
    del fb_idx
