...
class ara_matched_filter:
    ...
    def get_padded_wf(self, pad_v):
        ...
        self.roll_one_pad = (maximum_filter1d((~self.bool_pad).astype(int), axis = 0, size = self.roll_win_idx, mode='constant') != 1).astype(float)
    ...
    def get_evt_wise_corr(self):
        ...
        if self.use_debug:
            ...
            self.norm_sum_indi = np.full((self.lag_len, num_ants, self.num_temp_params[0], self.num_arr_params[0], self.num_arr_params[1]), 0, dtype = float)
            self.norm_sum_indi_roll = np.copy(self.norm_sum_indi)
            self.norm_sum_indi_off = np.full((num_ants, self.num_temp_params[0], self.num_arr_params[0], self.num_arr_params[1]), np.nan, dtype = float)
            ...
            for theta in range(self.num_arr_params[0]):
                ...
                for phi in range(self.num_arr_params[1]):
                    ...
                    if arr_idx < 0:
                        ...
                        norm_sum[-arr_idx:, self.good_ch_pol[ant], :, theta, phi] += self.roll_one_pad[:arr_idx, ant][:, np.newaxis] * norm_ch[np.newaxis, :]
                    elif arr_idx > 0:
                        corr_sum[:-arr_idx, self.good_ch_pol[ant], :, theta, phi] += corr_ch[arr_idx:]
                        norm_sum[:-arr_idx, self.good_ch_pol[ant], :, theta, phi] += self.roll_one_pad[arr_idx:, ant][:, np.newaxis] * norm_ch[np.newaxis, :]
                    else:
                        corr_sum[:, self.good_ch_pol[ant], :, theta, phi] += corr_ch
                        norm_sum[:, self.good_ch_pol[ant], :, theta, phi] += self.roll_one_pad[:, ant][:, np.newaxis] * norm_ch[np.newaxis, :]
                    if self.use_debug:
                        self.corr_sum_indi_off[self.good_chs[ant], :, theta, phi] = self.off_bin[off_max_idx[ant, :, self.res_theta_idx[theta]]]
                        self.norm_sum_indi_off[self.good_chs[ant], :, theta, phi] = norm_fac_2nd[ant, :, self.res_theta_idx[theta]]
                        if arr_idx < 0:
                            ...
                            self.norm_sum_indi[-arr_idx:, self.good_chs[ant], :, theta, phi] = (~self.bool_pad[:arr_idx, ant]).astype(float)[:, np.newaxis] * norm_ch[np.newaxis, :]
                            self.norm_sum_indi_roll[-arr_idx:, self.good_chs[ant], :, theta, phi] = self.roll_one_pad[:arr_idx, ant][:, np.newaxis] * norm_ch[np.newaxis, :]
                        elif arr_idx > 0:
                            self.corr_sum_indi[:-arr_idx, self.good_chs[ant], :, theta, phi] = corr_ch_2nd[arr_idx:]
                            self.corr_sum_indi_roll[:-arr_idx, self.good_chs[ant], :, theta, phi] = corr_ch[arr_idx:]
                            self.norm_sum_indi[:-arr_idx, self.good_chs[ant], :, theta, phi] = (~self.bool_pad[arr_idx:, ant]).astype(float)[:, np.newaxis] * norm_ch[np.newaxis, :]
                            ...
                            self.norm_sum_indi[:, self.good_chs[ant], :, theta, phi] = (~self.bool_pad[:, ant]).astype(float)[:, np.newaxis] * norm_ch[np.newaxis, :]
                            self.norm_sum_indi_roll[:, self.good_chs[ant], :, theta, phi] = self.roll_one_pad[:, ant][:, np.newaxis] * norm_ch[np.newaxis, :]
                    del arr_idx
                del corr_ch, norm_ch
        if self.use_debug:
            self.corr_roll_sum = np.copy(corr_sum) # sum of each pol
        corr_sum /= np.sqrt(norm_sum)
        corr_sum[np.isnan(corr_sum) | np.isinf(corr_sum)] = 0
        if self.use_debug:
            self.norm_sum = np.copy(norm_sum)
            self.norm_sqrt = np.sqrt(self.norm_sum) 
            self.corr_roll_sum_norm = np.copy(corr_sum) # sum of each pol
        del corr_roll_max, norm_sum, norm_fac_2nd
