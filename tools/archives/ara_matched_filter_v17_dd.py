...
class ara_matched_filter:
    ...
    def get_template(self, use_sc = False):
        ...
        sc_phase = sc_p[:, 1]
        sc_phases = interp1d(sc_freq_phase, sc_phase, fill_value = 'extrapolate')

        int_freq = np.fft.rfftfreq(len(self.temp[:, 0,0,0,0]), self.dt)
        int_sc_phase = sc_phases(int_freq)
        int_sc_com = np.exp((0 + 1j) * int_sc_phase)

        int_fft = np.fft.rfft(self.temp, axis = 0)
        int_fft /= int_sc_com[:, np.newaxis, np.newaxis, np.newaxis, np.newaxis]
        self.temp = np.fft.irfft(int_fft, axis = 0, n = len(self.temp[:, 0,0,0,0]))
