...
class pre_qual_cut_loader:
    ...
    def get_zero_pedestal_events(self):
        ...
        zero_ped_evts = np.full((len(self.evt_num)), 0, dtype = int)
        for evt in tqdm(range(len(self.irs_block_number))):
    ...
    def run_pre_qual_cut(self):
        ...
        tot_pre_qual_cut[:,10] = self.get_zero_pedestal_events()
