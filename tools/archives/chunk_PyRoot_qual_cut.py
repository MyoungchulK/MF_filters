...
def wf_collector_dat(Data, Ped, Station,
    ...
    file, evtTree, rawEvt, num_evts, cal, q, ch_index, pol_type = ara_raw_to_qual(R, Data, Ped, Station, trig_info = trig_info, pol_info = pol_info, ant_ch = num_Ants, yrs = Year)
    del Ped
    ...
    evt_num, act_evt_num, unix_time, trig_num, trig_chs, time_stamp, read_win, hasKeyInFileError = uproot_loader(Data, Station, event_i, event_f, ch_index,
                                                                                            trig_set = trig_set, act_evt_info = act_evt_info, 
                                                                                            trig_info = trig_info, time_stamp_info = time_stamp_info, 
                                                                                            read_win_info = read_win_info, unix_info = unix_info)
    del Data
    ...
    if raw_hill_dat == True:
        ...
        raw_hill_all = None
    qual_num = np.full((evt_num_len), -1, dtype=int)
    ...
    for evt in tqdm(range(evt_num_len)):
        ...
        ant_arr, int_time_len, wf_if_arr, peak_arr, rms_arr, hill_arr, raw_time_len, raw_wf_if_arr, raw_peak_arr, raw_rms_arr, raw_hill_arr = station_pad(usefulEvent, num_Ants, dt_ns, 
    ...
    #output
    return wf_all, wf_len_all, wf_if_all, peak_all, rms_all, hill_all, raw_wf_len_all, raw_wf_if_all, raw_peak_all, raw_rms_all, raw_hill_all, evt_num, act_evt_num, ch_index, pol_type, trig_num, trig_chs, qual_num, unix_time, time_stamp, read_win, final_chunk
