...
def raw_wf_collector_dat(Data, Ped, Station, Run, Year, num_Ants = antenna_info()[2]):
    ...
    rf_trig = np.where(trig_type == 1)[0]
    ...
    del blk_mean_max_arr, blk_idx_max_arr
    ...
    rf_path = f'/data/user/mkim/OMF_filter/ARA0{Station}/Off_Blk/Off_Blk_A{Station}_R{Run}.h5'
    hf = h5py.File(rf_path, 'r')
    min_mu_wo_freq = hf['min_mu_wo_freq'][:]
    min_cov_mtx_wo_freq = hf['min_cov_mtx_wo_freq'][:]
    success_int_wo_freq = hf['success_int_wo_freq'][:]
    ...
    evt_idx_wo_freq_arr = blk_mean_corr[:,0] * freq_flag_temp[np.newaxis, :] 
    front_idx = 0
    for evt in tqdm(range(len(rf_evt_num))):
        ...
        front_idx += blk_len
        del blk_len
    del sig_wo_freq, front_idx, freq_flag_temp, evt_idx_wo_freq_arr 
 
    print('WF collecting is done!')

    #output
    return blk_mean_corr, rf_evt_num, ex_flag, freq_flag, min_mu_wo_freq, min_cov_mtx_wo_freq, sig_wo_freq_arr, ratio_wo_freq, success_int_wo_freq
