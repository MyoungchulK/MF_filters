...
def ped_collector(Data, Ped, analyze_blind_dat = False):
    ...
    for evt in tqdm(range(num_evts)):
        ...
        blk_idx = np.asarray(irs_block_number[evt][num_ddas:], dtype = int)
        ch_mask = np.asarray(channel_mask[evt][num_ddas:], dtype = int)
        dda_idx = np.asarray(dda_number[evt][num_ddas:], dtype = int)
    ...
    del num_eles, num_blks, num_chs, num_ddas, num_evts, irs_block_number, channel_mask, dda_number, bi_ch_mask, x_bins, y_bins
