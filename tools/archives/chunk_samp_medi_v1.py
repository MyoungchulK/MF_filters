...
def vol_calib_collector_dat(Data, Ped, Station, Year, evt_num, num_Ants = antenna_info()[2], save_wf = False):
    ...
    # output array
    wf_all = np.full((nsamp, len(amp_range), num_Ants), 0, dtype = int)
    medi_all = np.full((nsamp, num_Ants), np.nan)
    if save_wf == True:
        wf_mean_all = np.full((num_Ants, len(evt_num)), np.nan)
    ...
    for evt in tqdm(range(len(evt_num))):
        ...
        for ant in range(num_Ants):        
                ...
                if save_wf == True:
                    # wf mean
                    wf_mean_all[ant,evt] = np.nanmean(raw_wf)
                else:
                    pass 
                ...
                if np.any(np.abs(raw_wf_hist) > amp_range[-1]):
                    #print('high wf sample peak!', ant, evt_num[evt])
                ...
                # manual hist
                wf_all[chip_evt_idx[ant], amp_offset + raw_wf_hist, ant] += 1
        ...
        # Important for memory saving!!!!!!!
        del usefulEvent, chip_evt_idx, block_evt_idx

    # median cal.
    for sam in tqdm(range(nsamp)):
        for ant in range(num_Ants):

            cumsum = np.nancumsum(wf_all[sam, :, ant])
            ...
            if len(before_idx) > 0 and len(after_idx) > 0:
                medi_all[sam, ant] = (amp_range[before_idx[-1]] + amp_range[after_idx[0]])/2
            else:
                pass
            del before_idx, after_idx, cumsum, cumsum_mid

    del nsamp, R, ch_index, pol_type, ele_ch, file, evtTree, rawEvt, cal, useful_ch, amp_offset, amp_range
