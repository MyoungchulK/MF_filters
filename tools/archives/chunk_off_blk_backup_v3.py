...
def raw_wf_collector_dat(Data, Ped, Station, Year, num_Ants = antenna_info()[2]):
    ...
    # output array
    blk_est_range = 50
    blk_mean_corr = np.full((blk_est_range, num_Ants, len(rf_entry_num)), np.nan, dtype = float)
    evt_idx_arr = np.full((blk_est_range, len(rf_entry_num)), np.nan, dtype = float)

    blk_mean_max_corr = np.full((num_Ants, len(rf_entry_num)), np.nan, dtype = float)
    blk_idx_arr = np.copy(evt_idx_arr)
    blk_idx_max_arr = np.full((num_Ants, len(rf_entry_num)), np.nan, dtype = float)
    blk_mean_max_arr = np.copy(blk_idx_max_arr)
    ant_range = np.arange(num_Ants)

    blk_mean_sign = np.copy(blk_mean_corr)

    freq_flag = np.copy(blk_mean_max_corr)
    ...
    for evt in tqdm(range(len(rf_entry_num))):
        ...
        if len(blk_arr) == 0:
            print('zero block!', len(blk_arr), rf_entry_num[evt], rf_evt_num[evt])
            continue 
        ...
        for ant in range(num_Ants):        
            ...
            # mean of block
            mean_blk_arr[:,ant] = mean_blk_finder(raw_v, cap_num_arr[:,ant][cap_arr])

            blk_mean_sign[:len(blk_arr), ant, evt] = np.sign(mean_blk_arr[:,ant])

            if few_sample_error(raw_v, cap_num_arr[:,ant][cap_arr]):
                print('sample number issue!',rf_entry_num[evt], rf_evt_num[evt], ant)
                #continue
        ...
        max_idx = np.nanargmax(np.sqrt(np.nansum(mean_blk_arr**2, axis=1)))
        blk_mean_max_corr[:, evt] = mean_blk_arr[max_idx]

        blk_idx_arr[:len(blk_arr), evt] = local_blk_arr    
        ...
        # Important for memory saving!!!!!!!
        del usefulEvent, blk_arr, mean_blk_arr, max_idx, cap_arr, local_blk_arr, max_blk_idx
    ...
    # fluctuation check
    blk_mean_std = np.nanstd(blk_mean_corr, axis=0)
    blk_mean_diff = np.diff(blk_mean_sign, axis=0)
    blk_mean_diff_len = np.count_nonzero(np.abs(blk_mean_diff) > 1, axis = 0)
    ...
    # unwrap array
    sig_arr = np.full((blk_est_range, len(rf_entry_num)), np.nan, dtype = float)
    front_idx = 0
    for evt in tqdm(range(len(rf_entry_num))):
    ...
    #output
    return blk_mean_corr, evt_idx_arr, rf_entry_num, rf_evt_num, min_mu, min_cov_mtx, sig_arr, ratio, success_int, blk_mean_max_corr, blk_idx_arr, blk_idx_max_arr, blk_mean_max_arr, ex_flag, blk_mean_std, blk_mean_sign, blk_mean_diff, blk_mean_diff_len, freq_flag
