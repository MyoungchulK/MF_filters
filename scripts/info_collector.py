...
def info_loader(CPath = curr_path, Data = None, Ped = None, Output = None):
    ...
    # collecting sub-info.
    wf_len_all, wf_if_all, peak_all, rms_all, hill_all, entry_num, evt_num, ele_ch, trig_ch, pol_type, trig_type, trig_ant, unix_time, time_stamp, read_win, roll_mm, cliff_medi, freq_glitch, time_minus_idx = sub_info_collector_dat(Data, Ped, Station, Year)
    ...
    hf.create_dataset('time_minus_idx', data=time_minus_idx, compression="gzip", compression_opts=9)
    ...
    hf.create_dataset('trig_ant', data=trig_ant, compression="gzip", compression_opts=9)
