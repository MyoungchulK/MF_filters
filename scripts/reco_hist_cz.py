...
c_bins = np.linspace(0, 1.2, 120 + 1, dtype = float)
c_bin_center = (c_bins[1:] + c_bins[:-1]) / 2
c_bin_len = len(c_bin_center)

map_r_c = np.full((d_len, c_bin_len, 3, 2, 2, 2), 0, dtype = int) # run, z or a, trig, pol, rad, sol
...
c_tot = np.full((c_bin_len, 3, 2, 2, 2, num_configs), 0, dtype = int) # c, trig, pol, rad, sol, config
c_cut = np.copy(c_tot)
z_tot = np.full((z_bin_len, 3, 2, 2, 2, num_configs), 0, dtype = int) # z, trig, pol, rad, sol, config
z_cut = np.copy(z_tot)

cz_tot = np.full((c_bin_len, z_bin_len, 3, 2, 2, 2, num_configs), 0, dtype = int) # a, z, trig, pol, rad, sol, config
cz_cut = np.copy(map_tot)
...
file_name = f'Reco_Map_New_2d_A{Station}_R{count_i}.h5'
