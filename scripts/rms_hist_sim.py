...
rms_all = np.full((d_len, p_bin_len, 16, 2), 0, dtype = int)
print(rms_all.shape)
del p_bin_len, d_len
...
for r in tqdm(range(len(d_run_tot))):
  ...
  if r >= count_i and r < count_ff:
    ...
    for a in range(16):
        rms_all[r, :, a, 0] = np.histogram(snr[a], bins = r_bins)[0].astype(int)
        rms_all[r, :, a, 1] = np.histogram(snr_cut[a], bins = r_bins)[0].astype(int)
    del snr, snr_cut
...
file_name = f'RMS_Sim_A{Station}_R{count_i}.h5'
