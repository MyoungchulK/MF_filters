...
from tools.chunk_std import std_collector_dat

def std_loader(Data = None, Ped = None, Output = None):

    # collecting info
    Station, Run, Config, Year, Month, Date = data_info_reader(Data)

    # collecting wf
    results = std_collector_dat(Data, Ped)
    for r in results:
    ...
    if not os.path.exists(Output):
        os.makedirs(Output)
    os.chdir(Output)
    h5_file_name = f'Std_A{Station}_R{Run}'
    h5_file_name += f'.h5'
...
if __name__ == "__main__":
    ...
    <Output path ex)/data/user/mkim/OMF_filter/ARA02/Std/>
    ...
    std_loader(Data = data, Ped = ped, Output = output)
