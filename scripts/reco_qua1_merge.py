...
# sort
m_path = os.path.expandvars("$OUTPUT_PATH") + f'/ARA0{Station}/reco_ele_lite_q3{b_name}/'
ml_path = os.path.expandvars("$OUTPUT_PATH") + f'/ARA0{Station}/reco_ele_lite_q4{b_name}/'
r_path = os.path.expandvars("$OUTPUT_PATH") + f'/ARA0{Station}/reco_ele_lite_s1{b_name}/'
...
for r in tqdm(range(num_runs)):
    ...
    m_name = f'{m_path}reco_ele_lite_q3{b_name}_A{Station}_R{runs[r]}.h5'
    ml_name = f'{ml_path}reco_ele_lite_q4{b_name}_A{Station}_R{runs[r]}.h5'
    r_name = f'{r_path}reco_ele_lite_s1{b_name}_A{Station}_R{runs[r]}.h5'
    ...
    sel_evt_idx += sel_evt_idx_l.astype(int)
    del sel_evt_idx_l, coef_cal_l, coord_cal_l, coef_max_l, coord_max_l, coef_s_max_l, coord_s_max_l

    hf = h5py.File(r_name, 'w')
    hf.create_dataset('sel_evt_idx', data=sel_evt_idx, compression="gzip", compression_opts=9)
