...
def arr_time_table_loader(Station, Year):
    ...
    if Station == 2:
        radius_bin = np.linspace(2400, 3010, 10 + 1, dtype = float) # IC1S, IC22S
        theta_bin = 90 - np.linspace(40, 10, 200 + 1, dtype = float)
        phi_bin = np.linspace(-120, -80, 160 + 1, dtype = float)       
 
    if Station == 3:
        radius_bin = np.linspace(3230, 3710, 10 + 1, dtype = float) # IC1S, IC22S
        theta_bin = 90 - np.linspace(-35, 10, 180 + 1, dtype = float)
        phi_bin = np.linspace(-120, -80, 160 + 1, dtype = float)
    print(f'!!!!!! {radius_bin} !!!!!!!!')
    ...
    if not os.path.exists(Output):
        os.makedirs(Output)
    h5_file_name = f'{Output}arr_time_table_A{Station}_Y{Year}_spice.h5'
