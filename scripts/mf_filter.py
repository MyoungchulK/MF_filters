...
def ms_filter(Data, Ped, Station, Run, Output, DMode, CPath = curr_path, Sel_evt_soft = None, Sel_evt = None):
    ...
    if DMode == 'normal':
        ...
        del soft_psd, CPath, ROOT, file, eventTree, rawEvent, num_events, calibrator, qual, num_Antennas, time_pad_len, time_width_ns, time_pad_i, time_pad_f, freq, freq_w, temp_vol, num_theta, theta_w, peak_index, bad_ant_index

        # create output file
        os.chdir(Output)

        h5_file_name='Evt_Wise_SNR_A'+str(Station)+'_R'+str(Run)+'.h5'
        ...
        evt_w_snr, evt_w_snr_v, evt_w_snr_h, evts_num, trig, trig_index, ant_arr_copy, ant_arr_fft, ant_arr_fft_band, snr_wf_copy, snr_wf_copy_1, snr_wf_r_max_copy, v_match, h_match, v_sum_match, h_sum_match, v_sum_match_01, h_sum_match_01, v_avg_match, h_avg_match, evt_snr_v_sky_2d, evt_snr_h_sky_2d, nadir_range, phi_range, time_pad, mov_t, pad_t, peak_w, v_opt_angle, h_opt_angle = evt_snr_maker_debug(Station, Run, Output, CPath, Sel_evt # argv
                                    ...
                                    , soft_psd # psd
                                    , temp_vol, num_theta, theta_w, peak_index) # temp
        del CPath, ROOT, file, eventTree, rawEvent, num_events, calibrator, qual, num_Antennas, time_pad_len, time_width_ns, time_pad_i, time_pad_f, freq_w, temp_vol, num_theta, peak_index
        ...
        g2.create_dataset('WF_evt'+str(Sel_evt), data=ant_arr_copy, compression="gzip", compression_opts=9)
        g2.create_dataset('FFT_evt'+str(Sel_evt), data=ant_arr_fft, compression="gzip", compression_opts=9)
        g2.create_dataset('FFT_band_evt'+str(Sel_evt), data=ant_arr_fft_band, compression="gzip", compression_opts=9)
        ...
        g2.create_dataset('SNR_evt'+str(Sel_evt), data=snr_wf_copy, compression="gzip", compression_opts=9)
        ...
        g2.create_dataset('SNR_shift_roll_max_sum_evt'+str(Sel_evt)+'Hpol', data=h_sum_match, compression="gzip", compression_opts=9)
        g2.create_dataset('SNR_shift_roll_max_sum_count_evt'+str(Sel_evt)+'Vpol', data=v_sum_match_01, compression="gzip", compression_opts=9)
        g2.create_dataset('SNR_shift_roll_max_sum_count_evt'+str(Sel_evt)+'Hpol', data=h_sum_match_01, compression="gzip", compression_opts=9)
        g2.create_dataset('SNR_shift_roll_max_avg_evt'+str(Sel_evt)+'Vpol', data=v_avg_match, compression="gzip", compression_opts=9)
        g2.create_dataset('SNR_shift_roll_max_avg_evt'+str(Sel_evt)+'Hpol', data=h_avg_match, compression="gzip", compression_opts=9)
