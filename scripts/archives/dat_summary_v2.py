...
m_bins = np.linspace(0, 1000, 8000 + 1)
...
t_bin_len = len(theta_bin)
p_bin_len = len(phi_bin)

coef_max = np.full((d_len, c_bin_len, 3, 2), 0, dtype = int) # run, bins, trig, pol
coef_max_cut = np.copy(coef_max)
coef_
coef_max_az = np.full((num_configs, a_bin_len, z_bin_len, 3, 2), 0, dtype = int) # config, a, z, trig, pol
coef_max_zc = np.full((num_configs, z_bin_len, c_bin_len, 3, 2), 0, dtype = int) # config, z, c, trig, pol
coef_max_ac = np.full((num_configs, a_bin_len, c_bin_len, 3, 2), 0, dtype = int) # config, a, c, trig, pol
coef_max_az_cut = np.copy(coef_max_az)
coef_max_zc_cut = np.copy(coef_max_zc)
coef_max_ac_cut = np.copy(coef_max_ac)

mf_max = np.full((d_len, m_bin_len, 3, 2), 0, dtype = int) # run, bins, trig, pol
mf_max_cut = np.copy(mf_max)
mf_max_az = np.full((num_configs, p_bin_len, t_bin_len, 3, 2), 0, dtype = int) # config, a, z, trig, pol
mf_max_zc = np.full((num_configs, t_bin_len, m_bin_len, 3, 2), 0, dtype = int) # config, z, c, trig, pol
mf_max_ac = np.full((num_configs, p_bin_len, m_bin_len, 3, 2), 0, dtype = int) # config, a, c, trig, pol
mf_max_az_cut = np.copy(mf_max_az)
mf_max_zc_cut = np.copy(mf_max_zc)
mf_max_ac_cut = np.copy(mf_max_ac)
