...
coef = np.full((pol_len, rad_len, sol_len, evt_len), np.nan, dtype = float) # pol, rad, sol, evt
coord = np.full((ang_len, pol_len, rad_len, sol_len, evt_len), np.nan, dtype = float) # thepi, pol, rad, sol, evt
...
for r in tqdm(range(num_runs)):
    ...
    coef[:, :, :, run_idx] = coef_tot
    coord[:, :, :, :, run_idx] = coord_tot
...
file_name = f'Data_Summary_old_v16_A{Station}_R{count_i}.h5'
