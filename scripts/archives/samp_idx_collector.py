...
from tools.chunk_samp_idx import samp_idx_collector_dat

def mean_blk_loader(CPath = curr_path, Data = None, Ped = None, Output = None):

    # collecting info
    Station, Run, Config, Year, Month, Date = data_info_reader(Data)

    # collecting wf
    results = samp_idx_collector_dat(Data, Ped, Station, Year)
    for r in results:
    ...
    if not os.path.exists(Output):
        os.makedirs(Output)
    os.chdir(Output)
    h5_file_name = f'Samp_Idx_A{Station}_R{Run}'
    h5_file_name += f'.h5'
...
if __name__ == "__main__":
    ...
    mean_blk_loader(CPath = curr_path+'/..', Data = data, Ped = ped, Output = output)
