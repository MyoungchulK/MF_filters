...
coef_rf_ant1_cut = np.copy(coef_tot)
coef_rf_ant2_cut = np.copy(coef_tot)
...
for r in tqdm(range(len(d_run_tot))):
    ...
    coef_cw_ant1 = coef_r[(trig == 0) & (cw_cut[0] == 0)]
    snr_cw_ant2 = snr_r[(trig == 0) & (cw_cut[1] == 0)]
    coef_cw_ant2 = coef_r[(trig == 0) & (cw_cut[1] == 0)]
    for a in range(16):
        coef_rf_ant1_cut += np.histogram2d(snr_cw_ant1, coef_cw_ant1, bins = (snr_bins, coef_bins))[0].astype(int)
        coef_rf_ant2_cut += np.histogram2d(snr_cw_ant2, coef_cw_ant2, bins = (snr_bins, coef_bins))[0].astype(int)
...
hf.create_dataset('coef_rf_ant1_cut', data=coef_rf_ant1_cut, compression="gzip", compression_opts=9)
hf.create_dataset('coef_rf_ant2_cut', data=coef_rf_ant2_cut, compression="gzip", compression_opts=9)
hf.close()
