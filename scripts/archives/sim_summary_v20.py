...
z_3_flat = np.sin(np.radians(theta_3_flat)) * rad_3_flat
...
coord_r_max = np.full((d_len, len(ang_num) + 1, pol_len, rad_len, sol_len, num_evts), np.nan, dtype = float) # run, thepi, pol, rad, sol, evt
coef_max = np.full((d_len, pol_len, num_evts), np.nan, dtype = float) # run, pol, evt
coord_max = np.full((d_len, len(ang_num) + 2, pol_len, num_evts), np.nan, dtype = float) # run, thepir, pol, evt
...
for r in tqdm(range(d_len)):
    ...
    try:
        ...
        coord_r_max[r, 2] = np.sin(np.radians(coord_r_max[r, 0])) * rad[np.newaxis, :, np.newaxis, np.newaxis]
        ...
        coord_3_max[r, 3] = z_3_flat[coef_max_idx]
        ...
        coord_max[r, 3] = z_flat[coef_max_idx]
        ...
        coord_s_max[r, 3] = z_flat[coef_max_idx]
        ...
        coord_s10_max[r, 3] = z_flat[coef_max_idx]
...
file_name = f'Sim_Summary_{Type}_v17_A{Station}.h5'
