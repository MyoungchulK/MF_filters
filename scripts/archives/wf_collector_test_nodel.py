...
from tools.chunk import wf_collector_dat_v3
...
def wf_loader(CPath = curr_path, Data = None, Ped = None, Output = None):
    ...
    #wf_all, wf_len_all, evt_num, trig_num, qual_num, final_chunk, mem_u = wf_collector_dat(Data, Ped, Station, trig_set = 'all', qual_set = 'all', wf_len = True, evt_info = True)
    ...
    wf_all, wf_len_all, evt_num, trig_num, qual_num, final_chunk, mem_u = wf_collector_dat_v3(Data, Ped, Station, trig_set = 'all', qual_set = 'all', wf_len = True, evt_info = True)
    ...
    if not os.path.exists(Output):
        os.makedirs(Output)
    os.chdir(Output)
    h5_file_name=f'WF_A{Station}_R{Run}_v3_cobalt.h5'
