...
mf_noise_hist = np.full((2, mf_bin_len, config_len), 0, dtype = float)
mf_signal_hist = np.full((2, 3, mf_bin_len, config_len), 0, dtype = float)
...
for r in tqdm(range(len(d_run_tot))):
    ...
    mf_signal_hist[0, fla_idx, :, c_idx] += np.histogram(evt_wise[0], bins = mf_bins, weights = evt_rate[fla_idx, c_idx, r_idx])[0] 
    mf_signal_hist[1, fla_idx, :, c_idx] += np.histogram(evt_wise[1], bins = mf_bins, weights = evt_rate[fla_idx, c_idx, r_idx])[0] 
...
if not os.path.exists(path):
    os.makedirs(path)
os.chdir(path)
file_name = f'Reco_MF_W_Sim_A{Station}.h5'
