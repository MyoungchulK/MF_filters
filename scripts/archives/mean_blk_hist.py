...
blk_mean_hist = []
bp_blk_mean_hist = []
blk_mean_hist_1d = np.full((200, 16), 0, dtype = int)
bp_blk_mean_hist_1d = np.copy(blk_mean_hist_1d)
dda_volt = []
dda_curr = []
dda_temp = []
tda_volt = []
...
for r in tqdm(range(len(d_run_tot))):
    ...
    m_hist = hf['blk_mean_hist'][:]
    bp_m_hist = hf['bp_blk_mean_hist'][:]
    
    if d_run_tot[r] > 1430:

        blk_mean_hist_1d += m_hist
        bp_blk_mean_hist_1d += bp_m_hist

    blk_mean_hist.append(m_hist)
    bp_blk_mean_hist.append(bp_m_hist)
...
hf.create_dataset('blk_mean_hist_1d', data=np.asarray(blk_mean_hist), compression="gzip", compression_opts=9)
hf.create_dataset('blk_mean_hist', data=np.asarray(blk_mean_hist), compression="gzip", compression_opts=9)
hf.create_dataset('bp_blk_mean_hist_1d', data=np.asarray(bp_blk_mean_hist), compression="gzip", compression_opts=9)
hf.create_dataset('bp_blk_mean_hist', data=np.asarray(bp_blk_mean_hist), compression="gzip", compression_opts=9)
hf.close()
