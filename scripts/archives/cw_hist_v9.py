...
# sort
d_path = os.path.expandvars("$OUTPUT_PATH") + f'/OMF_filter/ARA0{Station}/cw_lite/*'
print(d_path)
...
if Station == 2:
    ...
    cut_arr[:,0] = np.array([0.1,  0.08, 0.08,  0.1, 0.12,  0.1, 0.08,  0.1, 0.06, 0.06, 0.06,  0.1, 0.08, 0.06, 0.06, 1000], dtype = float)
    cut_arr[:,1] = np.array([0.1,  0.08, 0.08, 0.08, 0.12,  0.1, 0.08,  0.1, 0.06, 0.06, 0.06,  0.1, 0.08, 0.06, 0.06, 1000], dtype = float)
    cut_arr[:,2] = np.array([0.1,  0.08, 0.08, 0.08, 0.12, 0.08, 0.08,  0.1, 0.06, 0.06, 0.06, 0.08, 0.08, 0.06, 0.06, 1000], dtype = float)
    cut_arr[:,3] = np.array([0.08, 0.06, 0.06, 0.06,  0.1, 0.08, 0.08, 0.08, 0.06, 0.06, 0.06, 0.06, 0.06, 0.06, 0.06, 1000], dtype = float)
    cut_arr[:,4] = np.array([0.08, 0.06, 0.06, 0.06,  0.1, 0.08, 0.06, 0.08, 0.06, 0.06, 0.06, 0.06, 0.06, 0.06, 0.06, 1000], dtype = float)
    cut_arr[:,5] = np.array([0.06, 0.06, 0.06, 0.06,  0.1, 0.08, 0.06, 0.08, 0.04, 0.06, 0.06, 0.06, 0.06, 0.04, 0.06, 1000], dtype = float)

if Station == 3:
    g_dim = 7
    ex_ch = 4
    cut_arr = np.full((16, g_dim), np.nan, dtype = float)
    """cut_arr[:,0] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    cut_arr[:,1] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    cut_arr[:,2] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    cut_arr[:,3] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    cut_arr[:,4] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    cut_arr[:,5] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    cut_arr[:,6] = np.array([, , , , , , , , , , , , , , ,], dtype = float)
    """
...
file_name = f'CW_Log_{d_type}_A{Station}_v7.h5'
