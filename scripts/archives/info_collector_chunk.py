...
def info_loader(CPath = curr_path, Data = None, Ped = None, Output = None):
    ...
    chunk_index = 0
    chunk_size = 5000
    wf_all, wf_len_all, evt_num, trig_num, qual_num, final_chunk = wf_collector_dat(Data, Ped, Station, trig_set = 'all', qual_set = 'all', wf_dat = False, wf_len = True, evt_info = True, chunk_size = chunk_size, chunk_i = chunk_index)
    if final_chunk == False: print('File is bigger than chunk! Starts chunk analysis!')
    else: pass

    while final_chunk == False:
        chunk_index += 1
        wf_all_c, wf_len_all_c, evt_num_c, trig_num_c, qual_num_c, final_chunk = wf_collector_dat(Data, Ped, Station, trig_set = 'all', qual_set = 'all', wf_dat = False, wf_len = True, evt_info = True, chunk_size = chunk_size, chunk_i = chunk_index)

        #wf_all = np.append(wf_all,wf_all_c,axis=2)
        wf_len_all = np.append(wf_len_all,wf_len_all_c,axis=1)
        evt_num = np.append(evt_num,evt_num_c,axis=0)
        trig_num = np.append(trig_num,trig_num_c,axis=0)
        qual_num = np.append(qual_num,qual_num_c,axis=0)
        del wf_all_c, wf_len_all_c, evt_num_c, trig_num_c, qual_num_c
