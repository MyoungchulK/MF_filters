...
map_tot = np.full((a_bin_len, z_bin_len, 3, 2, 2, 2), 0, dtype = int) # trig, pol, rad, sol
  ...
  if r >= count_i and r < count_ff:
    ...
    for t in range(3):
        for pol in range(2):
            for rad in range(2):
                for sol in range(2):       
                    map_tot[:, :, t, pol, rad, sol] += np.histogram2d(coord[pol, 1, rad, sol][t_list[t]], coord[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_daq[:, :, t, pol, rad, sol] += np.histogram2d(c_daq[pol, 1, rad, sol][t_list[t]], c_daq[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_fir[:, :, t, pol, rad, sol] += np.histogram2d(c_fir[pol, 1, rad, sol][t_list[t]], c_fir[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_dda[:, :, t, pol, rad, sol] += np.histogram2d(c_dda[pol, 1, rad, sol][t_list[t]], c_dda[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_rate[:, :, t, pol, rad, sol] += np.histogram2d(c_rate[pol, 1, rad, sol][t_list[t]], c_rate[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_l1[:, :, t, pol, rad, sol] += np.histogram2d(c_l1[pol, 1, rad, sol][t_list[t]], c_l1[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_short[:, :, t, pol, rad, sol] += np.histogram2d(c_short[pol, 1, rad, sol][t_list[t]], c_short[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_bad[:, :, t, pol, rad, sol] += np.histogram2d(c_bad[pol, 1, rad, sol][t_list[t]], c_bad[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_log[:, :, t, pol, rad, sol] += np.histogram2d(c_log[pol, 1, rad, sol][t_list[t]], c_log[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_wb[:, :, t, pol, rad, sol] += np.histogram2d(c_wb[pol, 1, rad, sol][t_list[t]], c_wb[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_pole[:, :, t, pol, rad, sol] += np.histogram2d(c_pole[pol, 1, rad, sol][t_list[t]], c_pole[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_un[:, :, t, pol, rad, sol] += np.histogram2d(c_un[pol, 1, rad, sol][t_list[t]], c_un[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_ped[:, :, t, pol, rad, sol] += np.histogram2d(c_ped[pol, 1, rad, sol][t_list[t]], c_ped[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
                    map_cut[:, :, t, pol, rad, sol] += np.histogram2d(c_cut[pol, 1, rad, sol][t_list[t]], c_cut[pol, 0, rad, sol][t_list[t]], bins = (a_bins, z_bins))[0].astype(int)
