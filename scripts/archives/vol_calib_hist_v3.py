...
# sort
d_path = f'/data/user/mkim/OMF_filter/ARA0{Station}/Vol_Calib_Samp_Medi_New_Calib/*'
...
samp_per_block = 64
block_per_dda = 512
blk_range = np.arange(block_per_dda)
...
vol_edge = 30
vol_range = np.arange(-1*vol_edge,vol_edge,0.1)
vol_bins, vol_bin_center = bin_range_maker(vol_range, len(vol_range))

wf_mean_hist = np.full((len(vol_bin_center), ant_num), 0, dtype = int)
wf_mean_hist_list = []
print(wf_mean_hist.shape)
...
for r in tqdm(range(len(d_run_tot))):
    ...
    if d_run_tot[r] in bad_runs:
        ...
        wf_mean_all = hf['wf_mean_all'][:]

        wf_mean = np.full((len(vol_bin_center), ant_num),0,dtype=int)
        medi_blk = np.full((block_per_dda, ant_num),np.nan)
        for a in range(ant_num):
            medi_all_blk = np.reshape(medi_all_ori[:,a],(block_per_dda, samp_per_block))
            medi_blk[:,a] = np.nanmean(medi_all_blk,axis=1)
            del medi_all_blk
            
            wf_hist = np.histogram(wf_mean_all[a],bins = vol_bins)[0]
            wf_mean[:,a] = wf_hist
            wf_mean_hist[:,a] += wf_hist
            del wf_hist

        kLatestCalib_medi_list.append(medi_blk)
        wf_mean_hist_list.append(wf_mean)
        del wf_mean_all
        ...
        for a in range(ant_num):
            ...
            kLatestCalib_isnan_loc_indi = isnan_loc[:,a] 
            kLatestCalib_medi[nsamp_range,amp_offset + medi_all[:,a],a] += 1
            kLatestCalib_medi[nsamp_range[kLatestCalib_isnan_loc_indi],amp_offset_arr[kLatestCalib_isnan_loc_indi],a] -= 1
            del kLatestCalib_isnan_loc_indi
...
hf.create_dataset('vol_range', data=vol_range, compression="gzip", compression_opts=9)
hf.create_dataset('vol_bins', data=vol_bins, compression="gzip", compression_opts=9)
hf.create_dataset('vol_bin_center', data=vol_bin_center, compression="gzip", compression_opts=9)
...
hf.create_dataset('wf_mean_hist', data=wf_mean_hist, compression="gzip", compression_opts=9)
hf.create_dataset('wf_mean_hist_list', data=wf_mean_hist_list, compression="gzip", compression_opts=9)
