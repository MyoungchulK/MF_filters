...
from tools.chunk_abs_max import raw_wf_collector_dat
...
def rms_peak_loader(CPath = curr_path, Data = None, Ped = None, Output = None):
    ...
    # collecting wf
    abs_max, act_evt, rf_entry_num = raw_wf_collector_dat(Data, Ped, Station, Year)
    print(abs_max.shape)
    print(act_evt.shape)
    print(rf_entry_num.shape)

    # create output dir
    if not os.path.exists(Output):
        os.makedirs(Output)
    os.chdir(Output)
    h5_file_name = f'Abs_Max_A{Station}_R{Run}'
    h5_file_name += f'.h5'
    hf = h5py.File(h5_file_name, 'w')

    #saving result
    hf.create_dataset('config', data=np.array([Station, Run, Config, Year, Month, Date]), compression="gzip", compression_opts=9)
    hf.create_dataset('rf_entry_num', data=rf_entry_num, compression="gzip", compression_opts=9)
    hf.create_dataset('abs_max', data=abs_max, compression="gzip", compression_opts=9)
    hf.create_dataset('act_evt', data=act_evt, compression="gzip", compression_opts=9)
    del Station, Run, Config, Year, Month, Date
    del abs_max, act_evt, rf_entry_num
...
if __name__ == "__main__":
    ...
    <Output path ex)/data/user/mkim/OMF_filter/ARA02/Abs_Max/>
