...
d_list_sort = []
for d in range(len(run_tot)):
    d_list_sort.append(d_list[run_index[d]])
print(d_list_sort)
...
for r in range(len(run_tot)):
    rr = np.where(run_range == run_tot[r])[0][0]
    dname = f'/data/user/mkim/OMF_filter/ARA0{Station}/Info/Info_A{Station}_R{run_tot[r]}.h5'   
    print(d_list_sort[r], run_tot[r]) 
    try:
        # info
        hf = h5py.File(dname, 'r')
    except OSError:
        print('OSERROR!',dname, run_tot[r])
        continue
...
if not os.path.exists(path):
    os.makedirs(path)
os.chdir(path)
hf = h5py.File(f'Info_Hist_2d_Test_A{Station}_2018.h5', 'w')
