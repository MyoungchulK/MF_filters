...
from tools.chunk_trig_info import trig_info_collector_dat

def trig_info_loader(Data = None, Ped = None, Output = None):

    # collecting info
    Station, Run, Config, Year, Month, Date = data_info_reader(Data)

    # collecting wf
    results = trig_info_collector_dat(Data, Ped)
    for r in results:
    ...
    if not os.path.exists(Output):
        os.makedirs(Output)
    os.chdir(Output)
    h5_file_name = f'Trig_Info_A{Station}_R{Run}'
    h5_file_name += f'.h5'
...
if __name__ == "__main__":
    ...
    <Output path ex)/data/user/mkim/OMF_filter/ARA02/Full_Data/Trig_Info/>
    ...
    trig_info_loader(Data = data, Ped = ped, Output = output)
