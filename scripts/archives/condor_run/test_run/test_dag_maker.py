...
# sort
n_path = f'/data/user/mkim/OMF_filter/ARA0{Station}/RMS_Peak_New/*'
n_list, n_run_tot, n_run_range = file_sorter(n_path)
del n_run_range

o_path = f'/data/user/mkim/OMF_filter/ARA0{Station}/RMS_Peak_Old/*'
o_list, o_run_tot, o_run_range = file_sorter(o_path)
del o_run_range
...
ped_list, ped_run = repeder_list_maker(f'/data/user/mkim/OMF_filter/ARA0{Station}/Ped/', n_run_tot)
...
dag_file_name = f'ARA0{Station}_test.dag'
contents = ""

for r in tqdm(range(len(n_run_tot))):

    if n_run_tot[r] not in o_run_tot:

        n_run_idx = np.where(data_run == n_run_tot[r])[0]
        if len(n_run_idx) > 0:
            n_run_list = data_list[n_run_idx[0]]
        else:
            print('There is no run!')
            print(n_run_tot[r])
            sys.exit(1)

        n_ped_idx = np.where(ped_run == n_run_tot[r])[0]
        if len(n_ped_idx) > 0:
            n_ped_list = ped_list[n_ped_idx[0]]
        else:
        ...
        print(n_ped_list)
        # write contents
        contents = dag_statement(r, n_run_list, n_ped_list, Station, n_run_tot[r])
