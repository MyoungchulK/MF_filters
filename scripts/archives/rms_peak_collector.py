...
from tools.chunk_rms_peak import raw_wf_collector_dat
...
def rms_peak_loader(CPath = curr_path, Data = None, Ped = None, Output = None):
    ...
    # collecting wf
    rms_all, peak_all = raw_wf_collector_dat(Data, Ped, Station, Year)
    print(rms_all.shape)
    print(peak_all.shape)

    # create output dir
    if not os.path.exists(Output):
        os.makedirs(Output)
    os.chdir(Output)
    h5_file_name = f'RMS_Peak_A{Station}_R{Run}'
    h5_file_name += f'.h5'
...
if __name__ == "__main__":
    ...
    <Output path ex)/data/user/mkim/OMF_filter/ARA02/RMS_Peak/>
