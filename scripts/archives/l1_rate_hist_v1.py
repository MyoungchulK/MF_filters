...
for r in tqdm(range(len(d_run_tot))):
    ...
    for l in range(16):
        ...
        l1_h_cut[:, l] = np.histogram(l1_r_cut[:, l], bins = rate_bins)[0].astype(int)
    l1_rate.append(l1_h)
    ...
    for a in range(16):
        ...
        l1_flu_h_cut[:, a] = np.histogram(l1_flu1_cut, bins = rate_bins)[0].astype(int)
        l1_mean_h_cut[:, a] = np.histogram(l1_mean1_cut, bins = rate_bins1)[0].astype(int)
...
if not os.path.exists(path):
    os.makedirs(path)
os.chdir(path)
file_name = f'L1_Rate_v3_A{Station}.h5'
