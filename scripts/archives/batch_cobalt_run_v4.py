...
def cobalt_run_loader(Station = None, Key = None, Act_Evt = None, analyze_blind_dat = False):
    ...
    for w in range(len(new_list)):
        ...
        if count >= Act_Evt[0] and count < Act_Evt[1]:
            #if Station == 3 and int(new_list[w]) == 499: continue

            CMD_line = f'python3 -W ignore script_executor.py -k qual_cut_1st -s {Station} -r {int(new_list[w])} -b {int(analyze_blind_dat)} -n 1'
            print(CMD_line)
            call(CMD_line.split(' '))
