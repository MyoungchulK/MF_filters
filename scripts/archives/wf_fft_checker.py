...
def wf_fft_checker(Data, Ped, Output, Sel_evt = None):
    ...
    # save event info
    g0.create_dataset('Config', data=np.array([Station, Run, Config, Year, Month, Date]), compression="gzip", compression_opts=9)
    ...
    for ant_i in range(num_Antennas+4):
    ...
    # raw wf plot
    plot_16_indi(r'Time [ $ns$ ]',r'Amplitude [ $mV$ ]',f'Raw WF, A{Station}, Run{Run}, Evt{Sel_evt}, \n Y{Year}, M{Month}, D{Date}, Config:{Config}, Trig:{trig}, Qual:{quality}, Bad Ant:{bad_ant_label}'
            ,raw_wf_t, raw_wf_v
