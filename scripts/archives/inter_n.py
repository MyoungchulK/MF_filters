...
dpath = '/data/user/mkim/Noise_Aeff/'

dname = 'AraOut.setup_random_N_P-3_N500_rayl_crop.txt.run'
...
for a in tqdm(range(runs)):
    ...
    wf_all[:,:,a*500:a*500+500] = wf_all_indi
    ...
    wf_len_all[:,a*500:a*500+500] = wf_len_all_indi
    del hf, wf_len_all_indi, wf_all_indi

print(wf_all.shape)
print(wf_len_all.shape)

spath = '/data/user/mkim/MF_Noise_Aeff/'
sname = 'AraOut.setup_random_N_P-3_N500_rayl_crop.txt.run0_tot.h5'

hf = h5py.File(spath+sname, 'r')
snr_max = hf['snr_max'][:,:2000]
del hf
print(snr_max.shape)
