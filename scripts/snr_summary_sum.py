...
# sort
d_path = os.path.expandvars("$OUTPUT_PATH") + f'/ARA0{Station}/Hist/SNR_Summary_A{Station}_R*'
...
num_ants = 16
snr = np.full((num_ants, 0), np.nan, dtype = float) # ch, evts
...
for r in tqdm(range(len(d_run_tot))):
    ...
    snr1 = hf['snr'][:]
    snr = np.concatenate((snr, snr1), axis = 1)
    del hf, snr1
...
file_name = f'SNR_Summary_A{Station}.h5'
