...
# sort
d_path = os.path.expandvars("$OUTPUT_PATH") + f'/ARA0{Station}/rpr/'
...
num_ants = 16
rpr = np.full((num_ants, evt_len), np.nan, dtype = float) # ch, evts 
...
for r in tqdm(range(num_runs)):
    ...
    r_name = f'{d_path}rpr_A{Station}_R{runs_pa[r]}.h5'
    hf = h5py.File(r_name, 'r')
    rpr1 = hf['snr'][:] # ch, evt
    rpr1[bad_ant] = np.nan   
    rpr[:, run_idx] = rpr1 
    del bad_ant, run_idx, r_name, hf, rpr1
...
file_name = f'Data_Summary_RPR_A{Station}_R{count_i}.h5'
