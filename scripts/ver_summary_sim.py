...
mb_path = os.path.expandvars("$OUTPUT_PATH") + f'/ARA0{Station}/vertex_sim/'
...
theta = np.full((d_len, 3, num_evts), np.nan, dtype = float) # chs, evts
phi = np.copy(theta)
rr = np.copy(theta)
x = np.copy(theta)
y = np.copy(theta)
...
for r in tqdm(range(d_len)):
    ...
    try:
        hf = h5py.File(f'{mb_path}vertex{hf_name}', 'r')
        theta[r] = hf['theta'][:]
        phi[r] = hf['phi'][:]
        rr[r] = hf['r'][:]
        x[r] = hf['x'][:]
...
file_name = f'Sim_Summary_{Type}_Ver_A{Station}.h5'
...
hf.create_dataset('r', data=rr, compression="gzip", compression_opts=9)
